...
**Nom et prénom :** Charles-Edouard Tagba
...
**Q1.**1 Quelles sont les **trois zones** de Git sur ta machine ?
...
**Q2.**2 Que fait exactement `git add` ?
...
**Q3.**3 Quelle est la différence entre un commit et un envoi vers le distant ?
...
**Q4.**4 🛠️ Tu ajoutes un fichier déjà suivi par Git dans le `.gitignore`. Que se passe-t-il ?
...
**Q5.**5 Qu'est-ce qu'une fusion en avance rapide ?
...
**Q6.**6 Tu vois apparaître dans un fichier les marqueurs `<<<<<<<`, `=======` et `>>>>>>>`. Que dois tu faire ?
...
**Q7.**7 Pourquoi ne rebase t on pas une branche déjà partagée avec d'autres ?
...
**Q8.**8 Dans une paire de clés SSH, laquelle dépose t on sur GitHub ?
...
**Q9.**9 🛠️ Tu obtiens `Permission denied (publickey)` alors que « tout est bon côté GitHub ». Quelle est la cause la plus fréquente rencontrée en TP ?
...
**Q10.**10 🛠️ Ton envoi est refusé avec un message indiquant que le distant contient du travail que tu n'as pas. Quelle est la bonne réaction ?
...
**Q11.**11 À quoi sert une étiquette (`tag`) ?
...
**Q12.**12 Que produit concrètement une règle de protection sur la branche principale ?
...
**Q13.**13 Quelle est la différence de fond entre une machine virtuelle et un conteneur ?
...
**Q14.**14 Image et conteneur : quelle formulation est juste ?
...
**Q15.**15 Dans une publication de port notée `8001:8000`, que représente chaque nombre ?
...
**Q16.**16 🛠️ Ton conteneur tourne, la publication de port est correcte, mais aucune requête n'aboutit depuis ta machine. Quelle est la cause vue en cours ?
...
**Q17.**17 Pourquoi installe t on les dépendances **avant** de copier le code applicatif dans la recette de construction ?
...
**Q18.**18 À quoi sert le fichier d'exclusion de construction (`.dockerignore`) ?
...
**Q19.**19 Quel est l'intérêt d'une construction en plusieurs étages ?
...
**Q20.**20 Pourquoi éviter que le processus d'un conteneur tourne avec les privilèges du superutilisateur ?
...
**Q21.**21 Quelle est la différence entre un montage d'un dossier de l'hôte et un volume nommé ?
...
**Q22.**22 🛠️ Ta base de données démarre, mais le script d'initialisation n'a manifestement pas été rejoué et la table est absente. Pourquoi ?
...
**Q23.**23 Dans une pile décrite par un fichier de composition, comment l'API joint elle la base de données ?

- [ ] A. Par l'adresse `localhost`, puisque les deux conteneurs tournent sur la même machine
- [ ] B. Par l'adresse IP du conteneur, qu'il faut relever après chaque démarrage
- [x] C. Par le **nom du service**, résolu par le service de noms du réseau créé automatiquement
- [ ] D. Par un volume partagé entre les deux conteneurs

**Q24.**24 Que garantit une dépendance conditionnée par l'état de santé du service dont on dépend ?
...
**Q25.**25 Qu'est-ce que la boucle de réconciliation ?
...
**Q26.**26 Tu supprimes un pod créé par un déploiement. Que se passe t il, et pourquoi ?
...
**Q27.**27 Comment un service sait il à quels pods envoyer le trafic ?
...
**Q28.**28 🛠️ Ton service ne répond pas alors que les pods tournent, et la ressource qui liste les destinataires du service est vide. Quel est le diagnostic ?
...
**Q29.**29 Que faut il savoir sur la ressource utilisée pour stocker un mot de passe dans le cluster ?
...
**Q30.**30 Quelle est la différence entre une sonde de disponibilité et une sonde de vivacité ?
...
**Q31.**31 🛠️ Tes pods restent en échec de récupération d'image dans ton cluster local, alors que l'image existe bien sur ta machine. Quelle est la cause vue en TP ?
...
**Q32.**32 Qu'est-ce qu'une mise à jour progressive ?
...
**Q33.**33 Dans quel ordre dérouler la trousse de débogage enseignée en cours ?
...
**Q34.**34 Qu'est-ce que l'intégration continue ?
...
**Q35.**35 Pourquoi la description du pipeline doit elle vivre dans le dépôt plutôt que dans l'interface de l'outil ?
...
**Q36.**36 Pourquoi l'étape de test est elle placée avant la construction et la publication ?
...
**Q37.**37 🛠️ Pourquoi le fichier d'accès au cluster fourni à Jenkins doit il être exporté dans sa variante interne ?
...
**Q38.**38 À quoi sert, dans le pipeline, l'attente de la fin réelle du déploiement ?
...
**Q39.**39 Pourquoi passer par le gestionnaire d'identifiants plutôt que d'écrire le mot de passe du registre dans le fichier de pipeline ?
...
**Q40.**40 Quel risque a t on accepté en donnant au conteneur Jenkins l'accès au moteur Docker de la machine ?
