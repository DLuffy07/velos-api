...
1. Remplace `TON_PSEUDO` dans `k8s/api.yaml` et `Jenkinsfile` par ton identifiant Docker Hub/registre.
...
kubectl create secret generic velos-secret \
  ...
  --from-literal=database_url='postgresql://postgres:TON_MOT_DE_PASSE@velos-db:5432/velos'
...
```bash
kubectl set image deployment/velos-api api=TON_PSEUDO/velos-api:2.0
kubectl rollout status deployment/velos-api
